    ...
    S: D3DApp,
{
    ...
    let wc = WNDCLASSEXA {
        ...
        lpszClassName: s!("D3DApp_NKrhuap"),
        ..Default::default()
    };
...
fn sample_wndproc<S: D3DApp>(sample: &mut S, message: u32, wparam: WPARAM) -> bool {
    match message {
        WM_KEYDOWN => {
...
mod d3dapp_hello_triange {
    use super::*;

    const FRAME_COUNT: u32 = 2;

    pub struct D3DSample {
    ...
    fn populate_command_list(resources: &Resources) -> Result<()> {
        ...
        unsafe {
            command_list.ClearRenderTargetView(
                rtv_handle,
                &[0.1_f32, 0.1_f32, 0.15_f32, 1.0_f32],
                None,
            );
    ...
    fn create_vertex_buffer(
        ...
        let vertices = [
            Vertex {
                position: [0.0, 0.3 * aspect_ratio, 0.0],
                color: [0.275, 0.835, 1.0, 1.0],
            },
            Vertex {
                position: [0.4, -0.3 * aspect_ratio, 0.0],
                color: [0.875, 0.61, 0.930, 1.0],
            },
            Vertex {
                position: [-0.4, -0.3 * aspect_ratio, 0.0],
...
fn main() -> Result<()> {
    if let Err(e) = run_sample::<d3dapp_hello_triange::D3DSample>() {
